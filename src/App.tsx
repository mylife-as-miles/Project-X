...
export default function App() {
  ...
  const [scriptWidthPreset, setScriptWidthPreset] = useState<ScriptWidthPresetId>(() => {
    if (typeof localStorage !== 'undefined') {
      const saved = localStorage.getItem('project_x_script_width_preset');
  ...
  const [scrollFocusPreset, setScrollFocusPreset] = useState<ScrollFocusPresetId>(() => {
    if (typeof localStorage !== 'undefined') {
      const saved = localStorage.getItem('project_x_scroll_focus_preset');
  ...
  const [scriptThemeId, setScriptThemeId] = useState<ScriptThemeId>(() => {
    if (typeof localStorage !== 'undefined') {
      const saved = localStorage.getItem('project_x_script_theme');
      if (saved) {
  ...
  const applyScrollFocus = (presetId: ScrollFocusPresetId) => {
    ...
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('project_x_scroll_focus_preset', presetId);
    }
  ...
  useEffect(() => {
    if (isInitialized) {
      localStorage.setItem('project_x_state', JSON.stringify(state));
    }
  }, [state, isInitialized]);
  ...
  const resetState = () => {
    fetch('/examples/scene_frequency.json')
      ...
      .then(data => {
        ...
        setState(finalData);
        localStorage.setItem('project_x_state', JSON.stringify(finalData));
        setMode('playback');
        setCurrentTime(0);
  ...
  const exportJson = () => {
    ...
    downloadAnchorNode.setAttribute("download", "project_x.json");
    ...
    <div className="flex flex-col h-screen bg-stone-100 text-stone-900 font-sans overflow-hidden selection:bg-blue-100">
                              ...
                              return (
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    if (typeof localStorage !== 'undefined') {
                                      localStorage.setItem('project_x_script_width_preset', preset.id);
                                    }
      ...
      <ScriptColorModal
        ...
        onSelectTheme={(themeId) => {
          ...
          if (typeof localStorage !== 'undefined') {
            localStorage.setItem('project_x_script_theme', themeId);
          }
        }}
        cueTypes={COLORS}
      />
