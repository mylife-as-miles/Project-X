...
export interface ProjectExportData {
  version: "1.0";
  type: "projectx_project";
  exportedAt: string;
  project: Project;
  images?: { id: string; base64: string }[];
}

export const CURRENT_PROJECT_KEY = "projectx_current_project_id";
export const SYNC_CHANNEL_NAME = "projectx_project_sync_channel";
...
export function syncActiveProjectToLocalStorage(project: Project): void {
  ...
  try {
    ...
    window.dispatchEvent(new CustomEvent("projectx_project_switched", { detail: { projectId: project.id } }));
  } catch (err) {
...
export async function exportProjectJSON(projectId: string): Promise<void> {
  ...
  const filename = `projectx_${sanitizeName}_project_backup_${dateStr}_${timeStr}_${uniqueId}.json`;
...
export async function importProjectJSON(jsonString: string): Promise<Project> {
  ...
  if (parsed.type !== "projectx_project" || !parsed.project) {
    throw new Error("File is not a valid ProjectX Project export file.");
  }
