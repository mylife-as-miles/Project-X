...
export function exportAssetLibraryJSON(
  ...
  const filename =
    customFilename ||
    `projectx_${projectSlug}_asset_${exportType}_${dateStr}_${timeStr}_${uniqueId}.json`;
...
export async function readAndValidateAssetLibraryJSON(
  ...
  try {
    ...
    if (
      parsed &&
      parsed.type === "projectx_asset_library" &&
      ...
      return {
        success: false,
        error:
          "Invalid file format. The JSON file must be a valid ProjectX Asset Library export.",
      };
    }
