...
export const DB_NAME = "projectx_db";
  ...
  try {
    const activeId =
      ...
      (typeof localStorage !== "undefined"
        ? localStorage.getItem("projectx_current_project_id") ||
...
export async function deleteStoredImage(id: string, forceDelete = false): Promise<void> {
  ...
  if (!forceDelete) {
    ...
    if (isReferenced) {
      console.info(
        `[ProjectX IndexedDB] Image ${id} is still referenced by another project workspace or history item. Preserving binary payload in database.`
      );
      return;
    }
  }
...
export async function deduplicateStoredImages(): Promise<{
  ...
  if (updatesToApply.length > 0 || hashesAssigned > 0) {
    ...
    console.info(
      `[ProjectX IndexedDB] Migration & Deduplication complete: ${dedupCount} duplicate payload(s) consolidated, ${hashesAssigned} contentHash(es) backfilled.`
    );
  }
