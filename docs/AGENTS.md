...
This document provides instructions for AI agents and developers who need to maintain, extend, or modify the Project X codebase.
...
- **Modular Design Tokens (`src/styles/tokens/`)**:
  ...
  - `src/index.css`: Semantic CSS custom properties defined in `:root` (`--app-bg`, `--surface`, `--border-main`, `--text-main`, `--overlay-bg`) and mapped into Tailwind CSS v4's `@theme` directive.
...
- **LocalStorage Keys**:
  - `'project_x_state'`: Core project data (video ID, script text, cues, timing settings).
  - `'project_x_script_theme'`: Active script viewer theme ID (`ScriptThemeId`).
  - `'project_x_script_width_preset'`: Active desktop script width preset (`ScriptWidthPresetId`).
  - `'project_x_scroll_focus_preset'`: Active desktop auto-scroll focus anchor (`ScrollFocusPresetId`).
...
Project X maintains a curated library of built-in projects across 4 categories: **AI Scenes**, **The Written Motion (TWM Anthology)**, **FRAME Series**, and **AI Clips**. When adding or updating an example in any category:
...
3. **Synchronize `PROJECT_X_CATALOGUE.md`**:
