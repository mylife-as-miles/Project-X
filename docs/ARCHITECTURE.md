...
Project X follows a modular, 5-layer architecture that separates script parsing, visual theme rendering, shared utilities, state management, and UI presentation.
...
- Reads/writes `AppState` to `localStorage` under key `project_x_state`.
...
- `scriptWidthPreset` → key `project_x_script_width_preset` (5 presets).
- `scrollFocusPreset` → key `project_x_scroll_focus_preset` (3 presets).
- `scriptThemeId` → key `project_x_script_theme` (6 themes).
