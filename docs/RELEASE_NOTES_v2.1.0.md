...
**ProjectX v2.1.0** introduces **multi-modal video references** (MP4 uploads + YouTube URLs), Markdown-rendered generation output, history diff comparison, asset library import/export, and a major architecture refactor extracting 9 reusable components from the monolithic workspace.
...
ProjectX now supports video references alongside images. Upload MP4 files (≤30 seconds, ≤35 MB) or add YouTube URLs — both map to `@videoN` annotations in your prompt templates and stream to Gemini as `inlineData` / `fileData` parts.
...
- 📋 **Full Changelog**: https://github.com/mylife-as-miles/Project-X/compare/v2.0.0...v2.1.0
