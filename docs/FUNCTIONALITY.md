...
Project X is a specialized workspace for synchronizing video playback with written screenplays and AI prompts, evaluating AI video model fidelity, and analyzing script-to-screen prompt adherence.

---

## 1. Dual Script Parsing & Formatting Heuristics

Project X seamlessly parses raw screenplay and technical Auteur Script text into structured layouts using deterministic heuristics and regex:
...
- **Staging Block Directives in Project X**:
...
### Local Persistence
All project states (`project_x_state`), theme preferences (`project_x_script_theme`), width presets (`project_x_script_width_preset`), and scroll focus settings (`project_x_scroll_focus_preset`) persist in `localStorage`.
...
- **Import**: Uploads any valid Project X JSON file and triggers automatic cue realignment.
...
For a complete and up-to-date list of all available Project X projects, release dates, video models, and shareable IDs, refer to the **[Project X Catalogue](../PROJECT_X_CATALOGUE.md)**.
