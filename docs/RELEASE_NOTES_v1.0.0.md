...
**ProjectX** is a creative workspace for drafting and iterating on AI prompt templates. Write system instructions and prompt templates with `{{ dynamic_variables }}`, attach reference images, and test against Google Gemini in real time. This initial release ships the full editing, generation, and persistence pipeline.
...
ProjectX parses `{{ variable_name }}` placeholders from your template and builds the input form on the fly. No configuration files, no hardcoded fields. The reserved keys work as follows:
...
The resulting images are stored in **IndexedDB** (`projectx_db`) to avoid browser localStorage quota limits (~5–10MB). Only metadata (id, label, mimeType) is kept in localStorage. Legacy images with inline Base64 are auto-migrated on app load.
...
Three example presets are included to demonstrate what's possible, but **ProjectX is designed for you to bring your own system instructions and templates.** Edit freely or start from scratch via the Configure Prompts panel.
...
**⭐ Star on GitHub** · [mylife-as-miles/Project-X](https://github.com/mylife-as-miles/Project-X)
<br>**🌐 Live Deployment** · [projectx.mylife-as-miles.my.id](https://projectx.mylife-as-miles.my.id)
<br>**📄 License** · MIT © [Taruma Sakti](https://github.com/mylife-as-miles)
