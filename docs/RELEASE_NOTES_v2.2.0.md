...
ProjectX graduates from a single-workspace tool to a full multi-project creative environment. Create, rename, switch, and delete independent workspaces — each with its own system instructions, prompt templates, custom presets, generation history, and asset library — all persisted in IndexedDB.
...
- 📤 **Project import/export with image bundling** — export entire workspaces as versioned JSON files (`projectx_project` v1.0) with all asset image blobs retrieved from IndexedDB; import validates payloads, restores images, and auto-resolves name collisions with incrementing suffixes
...
- 📄 **Unified export filename convention** — all export types (assets, history, presets, projects) follow `projectx_{projectSlug}_{feature}_{date}_{time}_{uniqueId}.json` with a shared `slugify()` helper; asset library JSON exports are now pretty-printed with 2-space indentation
...
- 📡 **Cross-tab project synchronization** via `BroadcastChannel` (`projectx_project_sync_channel`)
...
- 📄 **Unified export filename convention** — all exports follow `projectx_{projectSlug}_{feature}_{date}_{time}_{uniqueId}.json` with pretty-printed asset library JSON
...
- ⚡ **Unified export filename convention** — all export types use `projectx_{projectSlug}_{feature}_{date}_{time}_{uniqueId}.json` with shared `slugify()` helper; `projectName` prop threaded through components for project-aware filenames
...
- 📡 **Cross-tab synchronization** — if you have multiple browser tabs open with an older version of ProjectX, project-switch events broadcast from v2.2.0 tabs may not be understood by older tabs. Recommend closing all tabs and reopening after upgrading.
...
- 📋 **Full Changelog**: https://github.com/mylife-as-miles/Project-X/compare/v2.1.0...v2.2.0
