...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === "start") {
      ...
      const targetPath = path.join(os.tmpdir(), `projectx_${safeUploadId}.tmp`);
    ...
    if (action === "finish") {
      ...
      const safeUploadId = rawUploadId.replace(/[^a-zA-Z0-9_-]/g, "");
      tempFilePath = path.join(os.tmpdir(), `projectx_${safeUploadId}.tmp`);
    ...
    tempFilePath = path.join(tempDir, `projectx_upload_${Date.now()}_${sanitizeName}`);
