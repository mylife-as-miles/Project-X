# AI Developer & Agent Guidelines: ProjectX

Welcome to **ProjectX**. This document outlines the project architecture, design rules, core conventions, and extensibility guidelines to ensure that any AI agent or human developer can build upon, modify, or debug this application while maintaining its high craftsmanship standards.

---

## 1. Core Purpose & Concept

ProjectX is an elegant, single-page creative workspace designed to compile rich, structured prompt templates with customizable dynamic placeholders and multi-modal reference files (reference images mapped to `@imageX` and videos mapped to `@videoX` casting annotations). The compiled instructions are processed server-side via the modern `@google/genai` SDK using the **Gemini 3.5 Flash** model.

Key differentiators:
- **Zero Database / Hybrid Persistence**: Uses static filesystem templates for fallback config defaults, client-side `localStorage` for active session configurations, and local presets for quick workspace instantiation. Project data, configurations, generation history, and image blobs are persisted in **IndexedDB** (`projectx_db` v3) with `contentHash` indexing and transparent payload deduplication to avoid browser storage quota limits (`LocalStorage` legacy keys are automatically migrated and purged).
...
- **Client-Side Storage**: **IndexedDB** (`projectx_db` v3 → `images` store with `contentHash` index & `projects` object store) for image blob and project data persistence with transparent deduplication, avoiding browser localStorage quota constraints.
...
├── /public
│   ├── logo_projectx.png           # Application logo
...
ProjectX is crafted in an **Analog Brutalist Retro Lab** aesthetic. Any new components, menus, modal dialogs, or inputs must align strictly with the following stylebook:
...
- **Gemini Files API Upload Paths**: Two upload paths are available for large media (images and videos up to 2 GB per file) via the "FILES API UPLOAD" modal (`AddFilesApiModal.tsx`):
...
- **Client-side dual-mode rendering**: The `GenerationResultView` component (`components/GenerationResultView.tsx`) renders output in two user-selectable modes:
  - **Formatted Markdown**: Uses `react-markdown` with custom component overrides (headings, paragraphs, lists, inline code blocks, bold, italic, horizontal rules) styled in the ProjectX brutalist aesthetic.
...
- **Image Blob Storage**: Raw Base64 image data is stored in IndexedDB (`projectx_db` v2, `images` object store with `id` keyPath) rather than localStorage, avoiding browser storage quota limits (~5-10MB for localStorage vs hundreds of MB for IndexedDB).
...
- **IndexedDB Storage**: Asset images are stored in IndexedDB (`projectx_db` / `images` store) with metadata (label, mimeType, createdAt, isPinned, isFavorite) persisted in localStorage under `projectx_asset_library_images`.
...
- **History Card Summary**: A reusable `HistoryCardSummary` component (`components/HistoryCardSummary.tsx`) renders a detailed preview card for history items showing timestamp (standardized 24-hour format), media badges (IMG/VID counts with `ImageIcon` / `VideoIcon`), model badge (`Cpu` icon), preset badge (`Bookmark` icon in amber), title/idea excerpt, and a cleaned markdown-free output excerpt (truncated to ~220 characters). This component is integrated into `LoadWorkspaceConfirmModal`, `DeleteHistoryConfirmModal`, and `HistorySection` for consistent, rich history previews throughout the application.
...
- **Export Payload Format**: Exports produce a JSON file (`projectx_{projectSlug}_preset_{tag}_{date}_{time}_{uniqueId}.json`) conforming to the `PresetExportPayload` interface with version (`"1.0"`), type (`"projectx_presets_export"`), exportedAt timestamp, exportType, itemCount, and an array of `UserPreset` objects. The `projectSlug` prefix is derived from the active project name.
...
- **Project-Based Data Model**: Each workspace (project) is stored as a `Project` object in IndexedDB (`projectx_db` v2 `projects` store) with its own `systemPrompt`, `promptTemplate`, `customPresets` array, `history` array, and `assetLibrary` array. The `Project` interface is defined in `lib/projects.ts` alongside CRUD helpers (`getAllProjects`, `getProject`, `saveProject`, `deleteProject`).
...
- **ProjectManagerModal**: A full-screen modal (`components/ProjectManagerModal.tsx`) provides access to all project management operations: browse and search projects, create new projects (with optional "Copy from Current" to clone the active workspace), rename projects, switch between projects with confirmation when there is unsaved session data, export projects as versioned JSON files (`projectx_project` v1.0) that bundle project config with all image blobs from IndexedDB, import previously exported project files with duplicate name detection and image restoration, and delete projects with the active workspace automatically falling back to the next available project. Fallback initialization in `handleProjectsUpdated()` auto-creates a default workspace when zero projects exist.
- **Cross-Tab Synchronization**: A `BroadcastChannel` (`projectx_project_sync_channel`) broadcasts project switch, update, create, and delete events between browser tabs. The `broadcastProjectChange()` and `subscribeProjectChanges()` utilities in `lib/projects.ts` handle the messaging layer, and a listener in `app/page.tsx` reloads the active project state when notified.
- **Project Import/Export**: `exportProjectJSON()` packages the full project configuration plus all asset image blobs (retrieved from IndexedDB `images` store) into a downloadable JSON blob. `importProjectJSON()` validates the payload, restores images to IndexedDB, handles name collisions with incrementing suffixes, and saves the imported project. Exported filenames follow a unified lowercase convention: `projectx_{projectSlug}_project_backup_{date}_{time}_{uniqueId}.json`.
- **Current Project Tracking**: The active project ID is persisted in `localStorage` (`projectx_current_project_id`) and restored on next session load. If the stored ID is invalid or missing, the most recently updated project is selected as the default.
