...
ProjectX includes 10 built-in system presets stored in subdirectories under `/prompts/presets/[preset_id]/` (each containing `meta.json`, `system_prompt.txt`, and `prompt_template.txt`):
...
> **These presets are examples only.** ProjectX is designed for you to bring your own system instructions and prompt templates — edit freely or start from scratch via the **Configure Prompts** panel.
...
```
https://projectx.mylife-as-miles.my.id/?presetUrl=https://raw.githubusercontent.com/user/repo/main/my-preset.json
```
...
MIT © [Taruma Sakti](https://github.com/mylife-as-miles)
