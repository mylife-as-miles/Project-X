...
- **Adaptive Dynamic Theme Logo (`src/index.css`, `src/components/*`)**:
  - Implemented zero-JS-lag CSS selectors (`.logo-light`, `.logo-dark`) to automatically display `/PROJECT_X_TAG_B.png` in Light/Warm modes and `/PROJECT_X_TAG_WHITE.png` in Dark mode across `AppHeader`, `AppInfoModal`, and `InitializingScreen`.
...
- **Comprehensive Documentation Refresh & Specification Synchronization**:
  ...
  - `docs/FUNCTIONALITY.md` — detailed dual script parsing heuristics (Screenplay & Auteur Script), updated modular staging scaffold specifications, and linked catalogue reference to `PROJECT_X_CATALOGUE.md`.
  - `docs/ARCHITECTURE.md` — documented Auteur Brief and state transition parsing within the Processing Layer (`scriptProcessor.ts`).
  - `PROJECT_X_CATALOGUE.md` — updated catalogue index to reflect latest AI clip examples and models.
...
- **Custom React Hook Architecture**: Extracted application business logic, playback controls, and editor state from `App.tsx` into a modular suite of eight custom hooks in `src/hooks/`:
  - `useScriptStorage` — encapsulates state initialization, initial default script loading (`scene_frequency.json`), `localStorage` persistence (`project_x_state`), starter blank script loading (`blank.json`), and CORS-aware remote project loading (`?project=URL`) with error handling.
  ...
  - `useScriptPreferences` — manages reading-column width presets, auto-scroll focus anchor presets, script theme selection, and cue category visibility filtering with automatic `localStorage` synchronization (`project_x_script_theme`, `project_x_script_width_preset`, `project_x_scroll_focus_preset`).
...
- **Branded Loading & Initialization Screen**: Replaced the generic spinner with a branded `InitializingScreen` component displaying the Project X logo (`PROJECT_X_TAG_B.png`) with a subtle pulse animation and monospace status text, providing a polished first-load experience.
...
### Added
- **Official Project X GUIDE**: Released a comprehensive, interactive quick-start guide as the default "Blank" script project.
...
### Changed
- **Branding Update**: Rebranded from "Project X" to "Project X" with a custom SVG logo and updated metadata.
