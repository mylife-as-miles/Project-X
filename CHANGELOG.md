...
All notable changes to ProjectX, a playground for drafting and iterating on AI prompt templates.
...
- **IndexedDB Generation History Storage & Legacy LocalStorage Cleanup (`lib/history-storage.ts`, `lib/projects.ts`, `app/page.tsx`).**
  ...
  - Moved primary history persistence into IndexedDB (`projectx_db` → `projects` store), storing history directly within the active workspace project record to avoid browser `LocalStorage` quota limits (`QuotaExceededError`).
...
- **Version Number Display in Footer (`components/FooterStatusBar.tsx`).** The footer now dynamically displays the current application version from `package.json` (e.g., `ProjectX v2.4.2 by Taruma Sakti`), providing immediate version context without requiring the user to inspect the app binary.
...
- **Mobile Header & Visual Assets Responsive Layout Refinements.** Refactored `components/AppHeader.tsx`, `components/KofiButton.tsx`, and `components/VisualAssetsSection.tsx`:
  - **Mobile Header**: Row 1 presents ProjectX branding and Project Manager button (`MAIN WORKSPACE...`) with Quick Preset Selector hidden. Row 2 presents header action controls (**Support**, **Assets**, **Engine**, **Prompts**, **Clear**) as clean icon-only buttons stretching evenly across the viewport.
...
- **Project import/export with image bundling.** Projects can be exported as versioned JSON files (`projectx_project` v1.0) that bundle the full project configuration alongside all asset image blobs retrieved from IndexedDB via `exportProjectJSON()`. Exported files use a standardized naming convention (`projectx_{projectSlug}_project_backup_{date}_{time}_{uniqueId}.json`). Import via `importProjectJSON()` validates the payload, restores images to IndexedDB, and automatically handles project name collisions with incrementing suffixes.
...
- **Cross-tab project synchronization.** A `BroadcastChannel`-based sync mechanism (`projectx_project_sync_channel`) broadcasts project switch, update, create, and delete actions across open browser tabs. The `subscribeProjectChanges()` / `broadcastProjectChange()` utilities in `lib/projects.ts`, paired with listeners in `app/page.tsx`, keep the project dropdown and active workspace in sync across tabs.
...
- **Versioned export payload.** Exports produce a JSON file (`projectx_presets_{tag}_{date}_{time}_{uniqueId}.json`) conforming to the `PresetExportPayload` interface with version 1.0, export timestamp, type tag, item count, and an array of `UserPreset` objects.
...
- IndexedDB image storage (`projectx_db`) avoiding browser localStorage quota limits (5–10MB vs hundreds of MB).
